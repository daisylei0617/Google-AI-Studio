    ...
    <footer className="px-6 md:px-20 py-12 border-t border-white/5 bg-[#080808]">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
        <div className="text-center md:text-left">
          <p className="text-[10px] text-gray-500 tracking-[0.3em] font-bold uppercase mb-2">获取联系</p>
        ...
        <div className="flex gap-8 text-[10px] tracking-widest font-bold text-gray-500 uppercase">
          <button onClick={() => onNavigate(Page.HOME)} className="hover:text-white transition-colors">首页</button>
          <button onClick={() => onNavigate(Page.EXPERIENCE)} className="hover:text-white transition-colors">履历</button>
          <button onClick={() => onNavigate(Page.PROJECTS)} className="hover:text-white transition-colors">项目</button>
          <button onClick={() => onNavigate(Page.SKILLS)} className="hover:text-white transition-colors">技能</button>
        </div>
